...
sub confugreLine
{
	...
	elsif ($os_name eq 'linux')
	{
		...
		return ("OPENSSL_LIBDIR='$openSSL/lib' OPENSSL_INCDIR='$openSSL/include' OPENSSL_LIBS='-lssl -lcrypto' ./configure -prefix $root/qtbase-$platform -c++std c++11 -opensource -debug -confirm-license -no-icu -qt-xcb -nomake tests -nomake examples -no-harfbuzz -qt-pcre -qt-libpng -openssl-linked -I $openSSL/openssl-$platform/include -L $openSSL/openssl-$platform/lib");
	}
